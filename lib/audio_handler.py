...
class AudioHandler:
    @staticmethod
    def convert_wav_to_flac(file_path, root, name):
        try:
    ...
    def is_audio_allowed_to_convert(file_path):
        ...
        # TODO: 把所有的有损编码格式都写进去
        if codec_name == 'aac':
            print(f'有损音频不会转换，文件{file_path}')
            return False
        if sample_fmt in ALLOWED_SAMPLE_FMT:
            return True
        else:
            print(f'不支持的编码格式，编码格式为{sample_fmt}')
            return False
...
class Splitter:
    ...
    @staticmethod
    def split_flac_with_cue(file_path):
...
class CueParser:
    ...
    def paser_cue_data(file_path):
        ...
        album_info["TRACKTOTAL"] = str(len(tracks_info))
